...
enum Type { LADDERS, SNAKES, CARDS };
...
public:
	...
	void DeleteAllGameObjects();														  // and  Draws it in the new cell

	Cell* GetCellList(int, int);

	Card* getCardfrompos(CellPosition);
	...
	Player* GetNextPlayer(const CellPosition& position);  // Gets a Pointer to the first Ladder after the passed "position"
	...
	GameObject* GetSnakeorLadderObjFromCellPosition(const CellPosition& pos);  // This function takes a CellPosition as input, and returns a pointer to the GameObject
                                                                      // that exists at that position in the grid. If there is no game object at that position,
                                                                      // it returns a nullptr.

	bool isLadderOverlap(Grid* pGrid, CellPosition startPos, CellPosition endPos);      // This function checks if the given start position overlaps with any ladders on the grid.

	bool isSnakeOverlap(Grid* pGrid, CellPosition startPos, CellPosition endPos);       // This function checks if the given start position overlaps with any snakes on the grid.
	...
	bool FireAttack();
	bool PoisonAttack();
	bool LightningAttack(Player*ptr);
	...
	Card* getCardfromCellNum(int pos);

	//================Additional Functions=========================

	void SaveObjects(ofstream&, Type type);
};
