...
void Output::CreateDesignModeToolBar() const
{
	...
	MenuItemImages[ITM_DELETE_OBJECT] = "images\\Remove.jpg";
	...
	MenuItemImages[ITM_EXIT_D] = "images\\Menu_Exit.jpg";
...
void Output::DrawLadder(const CellPosition& fromCell, const CellPosition& toCell) const
{
	...
	if ((!fromCell.IsValidCell() && !toCell.IsValidCell()) || (toCell.GetCellNum() < fromCell.GetCellNum()) || (fromCell.HCell() != toCell.HCell())) {
		return;
	}
	else {
	...
	///TODO: Set pen color and width using the appropriate variables of UI_Info object (UI)
		...
		pWind->SetPen(UI.LadderColor, UI.LadderlineWidth);
		// Calculate the length of the ladder as the difference between the "from" cell's vertical coordinate and the "to" cell's vertical coordinate
		int Length_Of_Ladder = fromCell.VCell() - toCell.VCell();

		for (int i = 0; i < Length_Of_Ladder; i++)
		{
			int xh_1 = x12;
			int yh_1 = (y2 + (UI.CellHeight / 2) + (i * (UI.CellHeight)));
			int xh_2 = x34;
			int yh_2 = (y2 + (UI.CellHeight / 2) + (i * (UI.CellHeight)));
			pWind->DrawLine(xh_1, yh_1, xh_2, yh_2);
		}
		// Draw the ladder rungs for each row between the "from" and "to" cells
	}
}
...
void Output::DrawSnake(const CellPosition& fromCell, const CellPosition& toCell) const
{
	...
	/* If the "from" cell's horizontal coordinate is not the same as the "to" cell's horizontal coordinate
	OR if the "from" cell's number is less than the "to" cell's number, then return from the function*/

	if (fromCell.GetCellNum() < toCell.GetCellNum() || fromCell.HCell() != toCell.HCell()){
		return;
    }
	else{// Get the upper left corner coordinates of the fromCell and toCell
		...
		// The polygon is intended to be drawn in the same way as the snakes shown in the project statement

		// Define the x-coordinates of the polygon's four points, based on the variable x12 and xChange
		int points_X[] = { x12, x12 - xChange, x12, x12 + xChange };

		// Define the y-coordinates of the polygon's four points, based on the variable y1 and yChange
		int points_Y[] = { y1 + 10, y1 + 10 - yChange, (y1 + 10 - (2 * yChange)), y1 + 10 - yChange };


		///TODO: Draw the Polygon (diamond) representing the Snake's Head
		//       Check the snakes drawn in the project document and draw it the same way

		pWind->DrawPolygon(points_X, points_Y, 4);
	}
